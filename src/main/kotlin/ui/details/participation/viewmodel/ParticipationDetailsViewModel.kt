...
class ParticipationDetailsViewModel
    @Inject
    constructor(
        ...
        val initialParticipationList = MutableStateFlow<MutableList<Participation>>(mutableListOf())
        ...
        private fun setParticipation() {
            coroutineScope.launch {
                getParticipationsUseCase().collect {
                    ...
                    initialParticipationList.value = it.list.toMutableList()
        ...
        fun updateParticipation() {
            coroutineScope.launch {
                deleteParticipationsOfOutStudents()
                updateParticipationUseCase(requiredParticipation.value)
                // updateParticipationUseCase(projectParticipation.value)
            }
        }

        private suspend fun deleteParticipationsOfOutStudents() {
            val participationListToDelete =
                initialParticipationList.value.filter { participation ->
                    participation.studentId in outStudents.value.map { it.id }
                }
            if (participationListToDelete.isNotEmpty()) {
                deleteParticipationUseCase(participationListToDelete)
            }
        }
    }
